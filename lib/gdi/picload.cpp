...
PyObject *ePicLoad::getInfo(const char *filename)
{
	...
	if(exif->DecodeExif(filename))
	{
		if(exif->m_exifinfo->IsExif)
		{
			...
			int pos=0;
			list = PyList_New(23);
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(filename));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->Version));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->CameraMake));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->CameraModel));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->DateTime));
			PyList_SET_ITEM(list, pos++,  PyUnicode_FromFormat("%d x %d", exif->m_exifinfo->Width, exif->m_exifinfo->Height));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->FlashUsed));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->Orientation));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->Comments));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->MeteringMode));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->ExposureProgram));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->LightSource));
			...
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(tmp));
			sprintf(tmp, "%.2f", exif->m_exifinfo->Yresolution);
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(tmp));
			PyList_SET_ITEM(list, pos++,  PyLong_FromString(exif->m_exifinfo->ResolutionUnit));
		...
		{
			list = PyList_New(2);
			PyList_SET_ITEM(list, 0, PyLong_FromString(filename));
			PyList_SET_ITEM(list, 1, PyLong_FromString(exif->m_szLastError));
		}
		exif->ClearExif();
	}
	else
...
SWIG_VOID(int) loadPic(ePtr<gPixmap> &result, std::string filename, int x, int y, int aspect, int resize_mode, int rotate, unsigned int background, std::string cachefile)
{
	...
	if(background)
		PyTuple_SET_ITEM(tuple, 6,  PyLong_FromString("#ff000000"));
	else
		PyTuple_SET_ITEM(tuple, 6,  PyLong_FromString("#00000000"));
