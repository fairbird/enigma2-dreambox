...
from Components.config import ConfigIP, ConfigMacText, ConfigNumber, ConfigPassword, ConfigSelection, ConfigSubsection, ConfigText, ConfigYesNo, NoSave, config, getConfigListEntry, ConfigInteger
		...
		for p in plugins.getPlugins(PluginDescriptor.WHERE_NETWORKSETUP):
			...
			if callFnc is not None:
				...
				if "WlanPluginEntry" in p.fnc:  # Internally used only for WLAN Plugin.
					...
					menuEntryName = p.fnc["menuEntryName"](self.iface) if "menuEntryName" in p.fnc else _("Extended Setup...")
					menuEntryDescription = p.fnc["menuEntryDescription"](self.iface) if "menuEntryDescription" in p.fnc else _("Extended Networksetup Plugin...")
