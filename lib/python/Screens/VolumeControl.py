...
class VolumeAdjustSettings(Setup):
	def __init__(self, session):
		...
		self["offsetActions"] = HelpableActionMap(self, ["ColorActions", "TVRadioActions"], {
			...
			"toggleTvRadio": (self.keyAddService, _("Add a service to the Volume Offset list"))
...
class VolumeAdjustServiceSelection(ChannelSelectionBase):
	...
	def __init__(self, session, mode):
		...
		self.skinName = ["VolumeAdjustServiceSelection", "SmallChannelSelection", "mySmallChannelSelection", "ChannelSelection"]  # The screen "mySmallChannelSelection" is for legacy support only.
		...
		self["volumeServiceActions"] = HelpableActionMap(self, ["SelectCancelActions", "TVRadioActions"], {
			...
			"toggleTvRadio": (self.keyModeToggle, _("Toggle between the available TV and RADIO services"))
