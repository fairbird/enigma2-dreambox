...
class MultiBootManager(Screen):
	...
	def deleteImage(self):
		...
		if BoxInfo.getItem("HasChkrootMultiboot") and slot == "1" and current and not BoxInfo.getItem("hasUBIMB"):
			self.session.openWithCallback(self.disableChkrootAnswer, MessageBox, _("Are you sure you want to disable Chkroot Multiboot?"), simple=True, title=self.getTitle())
		else:
...
class ChkrootSlotManager(Setup):
	...
	def showDeviceSelection(self):
		def readDevicesCallback():
			...
			self.session.openWithCallback(self.deviceSelectionCallback, MessageBox, text=_("Select target device for slot creation"), list=choiceList, title=self.getTitle())
		self.readDevices(readDevicesCallback)
