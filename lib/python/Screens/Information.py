...
class DebugInformation(InformationBase):
	...
	def findLogFiles(self):
		...
		paths += [x for x in sorted(glob(join(logPath, "Enigma2_crash_*.log")), key=lambda x: isfile(x) and getmtime(x))]
		if paths:
			countLogs = len(paths)
			for index, path in enumerate(reversed(paths)):
		...
		paths += [x for x in sorted(glob(join(logPath, "Enigma2_debug_*.log")), key=lambda x: isfile(x) and getmtime(x))]
		if paths:
			countLogs = len(paths)
			for index, path in enumerate(reversed(paths)):
