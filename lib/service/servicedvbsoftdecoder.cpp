...
void eDVBSoftDecoder::onFirstCwReceived()
{
	...
	eDebug("[SoftDecoder] First CW received - starting decoder");
...
void eDVBSoftDecoder::onWaitForFirstDataTimeout()
{
	...
	eWarning("[SoftDecoder] CW timeout - starting decoder anyway");
...
void eDVBSoftDecoder::startDecoderOrBuffer()
{
	...
	{
		eDebug("[SoftDecoder] Pre-buffering %dms before decoder start", bufferTime);
...
{
	eDebug("[SoftDecoder] Pre-buffer complete - starting decoder");
	startDecoder();
}
...
void eDVBSoftDecoder::stop()
{
	...
	if (m_decoder)
	{
		eDebug("[SoftDecoder] Releasing decoder");
		m_decoder = nullptr;
	}
...
int eDVBSoftDecoder::setupRecorder()
{
	...
	if (wait_timeout == 0)
	{
		eDebug("[SoftDecoder] CW wait disabled - starting decoder immediately");
		startDecoderOrBuffer();
		return 0;
	}
	...
	{
		eDebug("[SoftDecoder] First CW already available - starting decoder");
		startDecoderOrBuffer();
		return 0;
	}
		...
		if (!m_source_handler.getProgramInfo(program) && !program.audioStreams.empty())
		{
			...
			if (!m_noaudio)
			{
				...
				if (m_record)
				{
					...
					eDebug("[SoftDecoder] Write threshold set to %zu KB (%s audio)",
